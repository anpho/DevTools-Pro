...
var Sandbox = {
	...
	Model : Backbone.Model.extend({
		...
		addHistory: function(item) {
			...
			else if (_.isFunction(item.result)) item.result = item.result.toString();
			else if (_.isObject(item.result)) item.result = this.stringify(item.result);
			else if (_.isUndefined(item.result)) item.result = "undefined";
			navigator.cascades.postMessage(item.result);
		...
		evaluate: function(command) {
			...
			navigator.cascades.postMessage(">"+item.command);
...
var console={
		log:function(d){
			navigator.cascades.postMessage(JSON.stringify(d));
		}
}