  ...
  function Parser() {
    ...
    this.get_content = function () { //function to capture regular content between tags
      ...
      while (this.input.charAt(this.pos) !== '<') {
        ...
        char_r = this.input.charAt(this.pos);
        this.pos++;
        this.line_char_count++;
        
        
        if (this.Utils.in_array(char_r, this.Utils.whitespace)) {
          if (content.length) {
            space = true;
          }
          this.line_char_count--;
        ...
        content.push(char_r); //letter at-a-time (or string) inserted to an array
      }
    ...
    this.get_tag = function () { //function to get a full tag and parse its type
      ...
      do {
        ...
        if (this.Utils.in_array(char_r, this.Utils.whitespace)) { //don't want to insert unnecessary space
          space = true;
          this.line_char_count--;
          continue;
        }
        ...
        if (char_r === "'" || char_r === '"') {
          if (!content[1] || content[1] !== '!') { //if we're in a comment strings don't get treated specially
            char_r += this.get_unformatted(char_r);
            space = true;
          }
        }
        
        if (char_r === '=') { //no space before =
          space = false;
        }
        
        if (content.length && content[content.length-1] !== '=' && char_r !== '>'
        ...
        content.push(char_r); //inserts character at-a-time (or string)
      } while (char_r !== '>');
